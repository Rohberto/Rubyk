...
export default function QuizClient() {
  ...
  const inp: React.CSSProperties = {
    ...
    border: '1px solid rgba(61,46,30,0.14)', borderRadius: 8,
      ...
      <div style={{ maxWidth: 680, width: '100%', textAlign: 'center' }}>
        <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, ease }}
          style={{
            ...
            background: 'var(--orange-pale)', border: '1px solid rgba(232,99,42,0.25)',
            borderRadius: 100, padding: '5px 16px 5px 10px',
            fontSize: 12, fontWeight: 500, color: 'var(--orange)', marginBottom: 28,
          }}>
          <span style={{ width: 6, height: 6, background: 'var(--orange)', borderRadius: '50%' }} />
          Free · 2 minutes · 10 questions
        </motion.div>

        <motion.h1 initial={{ opacity: 0, y: 32 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.85, ease, delay: 0.1 }}
          style={{
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, ease, delay: 0.35 }}
          ...
          <button onClick={() => setPhase('quiz')} style={{
            ...
            padding: '16px 40px', fontSize: 16, fontWeight: 500, cursor: 'pointer',
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.5, duration: 0.7 }}
          style={{
            ...
            marginTop: 56, paddingTop: 40, borderTop: '1px solid rgba(61,46,30,0.08)',
          }}>
          {['Clarity', 'Origin Story', 'Investor Narrative', 'Customer Messaging',
            ...
            <span key={cat} style={{
              ...
              background: 'var(--warm)', border: '1px solid rgba(61,46,30,0.09)',
      ...
      <div style={{ maxWidth: 720, width: '100%' }}>
        <div style={{ marginBottom: 40 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
            ...
            <span style={{ fontSize: 11, fontWeight: 500, textTransform: 'uppercase', letterSpacing: '1px', color: 'var(--orange)' }}>
              {q.category}
            </span>
          </div>
          <div style={{ height: 3, background: 'rgba(61,46,30,0.10)', borderRadius: 2, overflow: 'hidden' }}>
            <motion.div
        ...
        <AnimatePresence mode="wait" custom={direction}>
          <motion.div key={current} custom={direction} variants={slideVariants}
            initial="enter" animate="center" exit="exit" transition={{ duration: 0.4, ease }}>
            <h2 style={{
              fontFamily: 'var(--font-cormorant), Georgia, serif',
              fontSize: 'clamp(24px, 3vw, 36px)', fontWeight: 600,
              color: 'var(--dark)', letterSpacing: '-0.5px', lineHeight: 1.3, marginBottom: 36,
            }}>{q.question}</h2>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              {q.options.map((opt, i) => {
                ...
                return (
                  <motion.button key={i} onClick={() => choose(opt.score, i)}
                    ...
                    style={{
                      ...
                      opacity: isDeselected ? 0.4 : 1, transition: 'all 0.25s ease',
                    ...
                    <span style={{ fontSize: 15, fontWeight: 300, color: isSelected ? '#fff' : 'var(--mid)', lineHeight: 1.65 }}>
                      {opt.label}
                    </span>
                  </motion.button>
                )
    ...
    <main style={section}>
      ...
      >
        ...
        <h2 style={{
          ...
          color: 'var(--dark)', letterSpacing: '-1px', lineHeight: 1.1, marginBottom: 12,
        }}>
        ...
        <form onSubmit={handleEmailSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 12, textAlign: 'left' }}>
          <input
            ...
            onChange={e => setName(e.target.value)} required style={inp}
          ...
          {sendError && (
            <p style={{ fontSize: 13, color: '#e74c3c', margin: 0 }}>{sendError}</p>
          )}

          <button type="submit" disabled={sending} style={{
            background: sending ? 'rgba(232,99,42,0.6)' : 'var(--orange)',
            color: '#fff', border: 'none', borderRadius: 8, padding: '14px',
            fontSize: 15, fontWeight: 500, cursor: sending ? 'not-allowed' : 'pointer',
            fontFamily: 'var(--font-outfit), system-ui, sans-serif',
            transition: 'background 0.2s', marginTop: 4,
          }}
          onMouseEnter={e => { if (!sending) e.currentTarget.style.background = 'var(--dark)' }}
          onMouseLeave={e => { if (!sending) e.currentTarget.style.background = 'var(--orange)' }}
          >
            {sending ? 'Sending results…' : 'Show me my results →'}
          </button>
        </form>
  ...
  return (
    <main style={{ ...section, justifyContent: 'flex-start' }}>
      <div style={{ maxWidth: 720, width: '100%' }}>
        <AnimatePresence>
          <motion.div initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.9, ease }}>

            {/* Score card */}
            <div style={{
              background: 'var(--dark)', borderRadius: 20,
              padding: 'clamp(32px, 5vw, 52px)', marginBottom: 28,
              ...
              <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', flexWrap: 'wrap', gap: 24, position: 'relative', zIndex: 1 }}>
                <div>
                  <p style={{ fontSize: 11, fontWeight: 500, textTransform: 'uppercase', letterSpacing: '2px', color: 'rgba(255,255,255,0.35)', marginBottom: 12 }}>
                    Your storytelling tier
                  </p>
                  <div style={{
                    display: 'inline-flex', alignItems: 'center', gap: 10,
                    background: `${result.color}22`, border: `1px solid ${result.color}55`,
                    borderRadius: 100, padding: '6px 18px 6px 10px', marginBottom: 20,
                  }}>
                    <span style={{ width: 8, height: 8, borderRadius: '50%', background: result.color, display: 'inline-block' }} />
                    <span style={{ fontSize: 13, fontWeight: 500, color: result.color, letterSpacing: '0.3px' }}>{result.tier}</span>
                  </div>
                  <h2 style={{
                    fontFamily: 'var(--font-cormorant), Georgia, serif',
                    fontSize: 'clamp(26px, 3vw, 38px)', fontWeight: 600,
                    color: 'var(--cream)', letterSpacing: '-0.5px', lineHeight: 1.2, maxWidth: 420,
                  }}>{result.headline}</h2>
                </div>
                <div style={{
                  display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center',
                  width: 96, height: 96, borderRadius: '50%',
                  border: `2px solid ${result.color}`, background: `${result.color}15`, flexShrink: 0,
                }}>
                  <span style={{ fontFamily: 'var(--font-cormorant), Georgia, serif', fontSize: 32, fontWeight: 600, color: result.color, lineHeight: 1 }}>{totalScore}</span>
                  <span style={{ fontSize: 11, color: 'rgba(255,255,255,0.3)', marginTop: 2 }}>/ 30</span>
                </div>
              </div>

              <p style={{ fontSize: 16, fontWeight: 300, color: 'rgba(253,250,246,0.6)', lineHeight: 1.78, marginTop: 24, position: 'relative', zIndex: 1 }}>
                {result.description}
              </p>
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 16, marginBottom: 28 }}>
              <div style={{ background: '#fff', border: '1px solid rgba(61,46,30,0.08)', borderRadius: 16, padding: 28 }}>
                <div style={{ fontSize: 11, fontWeight: 500, textTransform: 'uppercase', letterSpacing: '1.5px', color: '#1a9e6e', marginBottom: 16, display: 'flex', alignItems: 'center', gap: 8 }}>
                  <span style={{ width: 16, height: 16, borderRadius: '50%', background: 'rgba(26,158,110,0.12)', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', fontSize: 10 }}>✓</span>
                  What's working
                </div>
                <ul style={{ listStyle: 'none', padding: 0, display: 'flex', flexDirection: 'column', gap: 10 }}>
                  {result.strengths.map((s, i) => (
                    <li key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: 10 }}>
                      <span style={{ width: 6, height: 6, borderRadius: '50%', background: '#1a9e6e', flexShrink: 0, marginTop: 5 }} />
                      <span style={{ fontSize: 14, fontWeight: 300, color: 'var(--mid)', lineHeight: 1.6 }}>{s}</span>
                    </li>
                  ))}
                </ul>
              </div>
              ...
              <div style={{ background: '#fff', border: '1px solid rgba(61,46,30,0.08)', borderRadius: 16, padding: 28 }}>
                <div style={{ fontSize: 11, fontWeight: 500, textTransform: 'uppercase', letterSpacing: '1.5px', color: 'var(--orange)', marginBottom: 16, display: 'flex', alignItems: 'center', gap: 8 }}>
                  <span style={{ width: 16, height: 16, borderRadius: '50%', background: 'rgba(232,99,42,0.12)', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', fontSize: 10, color: 'var(--orange)' }}>→</span>
                  Where to focus next
                </div>
                <ul style={{ listStyle: 'none', padding: 0, display: 'flex', flexDirection: 'column', gap: 10 }}>
                  {result.gaps.map((g, i) => (
                    <li key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: 10 }}>
                      ...
                      <span style={{ fontSize: 14, fontWeight: 300, color: 'var(--mid)', lineHeight: 1.6 }}>{g}</span>
                    </li>
                  ))}
                </ul>
              </div>
            ...
            {/* Breakdown */}
            <div style={{ background: '#fff', border: '1px solid rgba(61,46,30,0.08)', borderRadius: 16, padding: 28, marginBottom: 28 }}>
              <p style={{ fontSize: 11, fontWeight: 500, textTransform: 'uppercase', letterSpacing: '1.5px', color: 'var(--muted-text)', marginBottom: 20 }}>
                Question breakdown
              </p>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                {questions.map((q, i) => {
                  ...
                  return (
                    <div key={i}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 5 }}>
                        <span style={{ fontSize: 12, fontWeight: 400, color: 'var(--mid)' }}>{q.category}</span>
                        <span style={{ fontSize: 11, fontWeight: 500, color: score === 3 ? '#1a9e6e' : score === 2 ? 'var(--orange)' : 'var(--muted-text)' }}>
                          {score === 3 ? 'Strong' : score === 2 ? 'Developing' : 'Needs work'}
                        </span>
                      </div>
                      <div style={{ height: 4, background: 'var(--warm)', borderRadius: 2, overflow: 'hidden' }}>
                        <motion.div
                          initial={{ width: 0 }}
                          animate={{ width: `${pct}%` }}
                          transition={{ duration: 0.6, ease, delay: i * 0.05 }}
                          style={{ height: '100%', background: score === 3 ? '#1a9e6e' : score === 2 ? 'var(--orange)' : '#B4B2A9', borderRadius: 2 }}
                        />
                      </div>
                    </div>
                  )
            ...
            {/* CTAs */}
            <div style={{
              background: 'var(--orange-pale)', border: '1px solid rgba(232,99,42,0.14)',
              borderRadius: 16, padding: 28,
              display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 20,
            }}>
              <div>
                <h3 style={{ fontFamily: 'var(--font-cormorant), Georgia, serif', fontSize: 22, fontWeight: 600, color: 'var(--dark)', letterSpacing: '-0.3px', marginBottom: 4 }}>
                  Ready to level up?
                </h3>
              ...
              <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
                <a href={totalScore <= 22 ? CALENDLY_URL : GUIDE_URL}
                  ...
                  style={{
                    ...
                    borderRadius: 8, padding: '12px 22px', fontSize: 14, fontWeight: 500,
                    fontFamily: 'var(--font-outfit), system-ui, sans-serif', transition: 'background 0.2s',
                  }}
                  onMouseEnter={e => (e.currentTarget.style.background = 'var(--dark)')}
                  onMouseLeave={e => (e.currentTarget.style.background = 'var(--orange)')}
                >{result.cta}</a>
                <button onClick={restart} style={{
                  background: 'transparent', border: '1px solid rgba(61,46,30,0.15)',
                  color: 'var(--muted-text)', borderRadius: 8, padding: '12px 22px',
